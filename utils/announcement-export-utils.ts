...
import type { Announcement } from "@/db/schemas/announcement-schema"
import jsPDF from "jspdf"
import autoTable from "jspdf-autotable"
import * as XLSX from "xlsx"

export interface AnnouncementExportable extends Announcement {
...
export const exportAnnouncements = async (
  announcements: AnnouncementExportable[],
  options: ExportOptions
) => {
  const { format, includeFields, filename } = options
  const timestamp = new Date().toISOString().split("T")[0]
  const defaultFilename = `announcements-export-${timestamp}`

  const filteredData = announcements.map((item) => {
    ...
    includeFields.forEach((field) => {
      switch (field) {
        ...
        case "status":
          ...
          break
        case "audience":
          filtered["Audience"] = item.audience
          break
        case "pinned":
          filtered["Pinned"] = item.pinned ? "Yes" : "No"
          break
        case "publishedAt":
          ...
          break
        case "expiresAt":
          filtered["Expires At"] = item.expiresAt
            ? new Date(item.expiresAt).toLocaleDateString("en-RW")
            : "N/A"
          break
        case "createdByName":
          filtered["Created By"] = item.createdByName || "N/A"
          break
        case "createdAt":
  ...
  const summaryData: PdfSummaryItem[] = [
    { label: "Total Announcements", value: String(totalAnnouncements) },
    { label: "Published", value: String(publishedAnnouncements) },
  ]
  ...
  switch (format) {
    case "csv":
      ...
      break
    case "excel":
      exportToExcel(filteredData, announcements, filename || defaultFilename)
      break
    case "pdf":
...
const exportToPDF = async (
  ...
  doc.setFontSize(14)
  doc.setFont("helvetica", "bold")
  doc.text("Announcements Export", 14, 48)
