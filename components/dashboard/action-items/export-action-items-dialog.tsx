...
import {
  exportActionItems,
  type ActionItemExportable,
  type ExportOptions,
} from "@/utils/action-item-export-utils"
...
interface ExportActionItemsDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  actionItems: ActionItemExportable[]
  selectedActionItems?: ActionItemExportable[]
}
...
const exportFields = [
  ...
  { id: "ownerName", label: "Owner", default: true },
  { id: "ownerEmail", label: "Owner Email", default: false },
  { id: "meetingTitle", label: "Meeting", default: false },
  { id: "dueDate", label: "Due Date", default: true },
  { id: "createdAt", label: "Created Date", default: false },
]

export function ExportActionItemsDialog({
  open,
  onOpenChange,
  actionItems,
  selectedActionItems,
}: ExportActionItemsDialogProps) {
  ...
  const dataToExport =
    selectedActionItems && selectedActionItems.length > 0
      ? selectedActionItems
      : actionItems
  ...
  const handleExport = async () => {
    ...
    const exportPromise = new Promise<void>((resolve, reject) => {
      try {
        ...
        exportActionItems(dataToExport, options)
        resolve()
      } catch (error) {
        reject(error)
      }
    ...
    toast.promise(exportPromise, {
      loading: `Exporting ${exportCount} action items as ${format.toUpperCase()}...`,
      success: `Successfully exported ${exportCount} action items as ${format.toUpperCase()}`,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Export {exportCount} item{exportCount !== 1 ? "s" : ""} in your
            preferred format
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder={`action-items-export-${new Date().toISOString().split("T")[0]}`}
            />
          </div>
