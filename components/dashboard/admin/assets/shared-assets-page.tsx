...
// Visibility tabs - private is intentionally excluded
...
export default function SharedAssetsPage() {
  ...
  // Build visibility param - exclude private always
  const visibilityParam =
    visibilityTab === "all"
      ? ["public", "authenticated", "committee", "admin"]
      : [visibilityTab]
  ...
  const handleDelete = async (asset: Asset) => {
    if (
      !confirm(
        `Delete "${asset.name}"? This cannot be undone and will remove the file for everyone.`
      )
    )
      return
    try {
    ...
    <div className="container mx-auto space-y-8 py-0 sm:py-5">
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Filter by who can see each asset. Private files are never shown
            here.
          </CardDescription>
        </CardHeader>
              ...
              {VISIBILITY_TABS.map((tab) => (
                <TabsTrigger key={tab.value} value={tab.value}>
                  ...
                  <Badge
                    variant="secondary"
                    className={cn(
                      "ml-1.5 text-xs px-1.5",
                      visibilityTab === tab.value && "bg-primary/20"
                    )}>
                    {tab.value === "all"
                      ? totalAssets
                      : visibilityCounts[
                          tab.value as Exclude<AssetVisibility, "private">
                        ]}
                  </Badge>
                </TabsTrigger>
              ))}
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {totalAssets} Shared Asset{totalAssets !== 1 ? "s" : ""}
          </CardTitle>
          <CardDescription>
            {debouncedSearch
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="mb-6 max-w-sm text-muted-foreground">
          Upload a file and set its visibility to Public, All Members, Committee
          Only, or Admin Only to have it appear here.
        </p>
