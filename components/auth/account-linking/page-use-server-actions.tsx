...
export default function AccountLinkingPage() {
  ...
  const handleLinkSocial = async (provider: string) => {
    try {
      ...
      toast.promise(linkPromise, {
        ...
        success: () => `If the redirect does not happen, please try again.`,
  ...
  const handleSetPassword = async (e: React.FormEvent) => {
    ...
    const setPasswordPromise = setPasswordAction(password)
    ...
    toast.promise(setPasswordPromise, {
      ...
      error: (err: any) => err.error || "Failed to set password.",
    })

    try {
      const result = await setPasswordPromise
      if (!result.success) {
