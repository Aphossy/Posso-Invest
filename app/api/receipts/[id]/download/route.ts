...
function canAccessReceipt(
  receipt: { memberId: string; issuedBy: string | null },
  role: string | null,
  userId: string
) {
  if (receipt.memberId === userId || receipt.issuedBy === userId) return true
...
export async function GET(
  ...
  if (!user)
    return apiError("UNAUTHORIZED", "Sign in to download receipts.", 401)

  const { id } = await params
  const found = await receiptOperations.findById(id)
  if (!found) return apiError("NOT_FOUND", "Receipt not found.", 404)

  if (!canAccessReceipt(found, role, user.id)) {
    return apiError("FORBIDDEN", "You do not have access to this receipt.", 403)
  }

  if (!found.fileUrl) {
    return apiError("NOT_FOUND", "No file attached to this receipt.", 404)
  }
  ...
  receiptOperations.incrementDownloadCount(id).catch(() => {})
