...
function apiError(code: string, message: string, status: number) {
  return NextResponse.json(
    { success: false, data: null, error: { code, message, details: {}, help: null } },
    { status }
  )
}
...
export async function GET() {
  try {
    ...
    console.error("[share-outs:me] failed to load member share-outs", { error })
    return apiError(
      "INTERNAL_SERVER_ERROR",
      "We could not load your share-outs right now.",
      500
    )
  }
}
